/*
 * Event Map Class
 * @constructor Rootless.Map.Event
...
Namespace('Rootless.Map.Event');

Rootless.Map.Event = Rootless.Map.extend({
   /**
    *  Initializes the Google Maps API for event forms
   ...
   mapInit : function(){
       ...
       this.directionsDisplay = new google.maps.DirectionsRenderer();
        ...
        $(self._.el.rideForm).validate({
            invalidHandler: function(form, validator) {
                ...
                for (var i = 0 ; i < numberOfInvalidElements ; i++)
                {
                    ...
                    _gaq.push(['_trackEvent', 'events', 'validationError', invalidElementName]);
                }
            },
        ...
        var event = Rootless.Map.Event.getInstance();
        var utils = Rootless.Static.Utils.getInstance();
        utils.signInDialogInit(event.MaybeSubmitForm);
        $(event._.el.loginFormDialogContainer).dialog({ autoOpen: false, modal : true, zIndex: 2000});
         
        // Show the event marker by previewing the route
        self.previewRoute($(self._.el.destinationTextBox));
        
        // Show the ride form and hide the confirmation section
        $(event._.el.eventFormBox).show();
        $(event._.el.eventRideConfirmationContainer).hide();
        ...
        $(self._.el.trackableField).change(function(){
            ...
            _gaq.push(['_trackEvent', 'events', 'changeFormField', $(this).attr('name')]);
        });
    ...
    formAjaxOptions : {
        ...
        beforeSubmit : function() {
            ...
            _gaq.push(['_trackEvent', 'events', 'createRideSubmitted']);
        },
        success: function(data)
        {
            ...
            // Unblock the form
            $(event._.el.eventRideContainer).unblock();

            // This handler function will run when the form is complete
            $(event._.el.eventFormBox).hide();
            $(event._.el.eventRideConfirmationContainer).show('blind');
            
            // Send an event to google analytics to show the form was submitted properly
            _gaq.push(['_trackEvent', 'events', 'createRideSuccess']);
        },
        error : function(xhr, status, errMsg)
        {
            ...
            if (xhr.status == 401)
            {
                ...
                $(event._.el.loginFormDialogContainer).dialog("open");
                
                // Send an event to google analytics for the form submission
                _gaq.push(['_trackEvent', 'events', 'createRideUnauthenticated']);
            }
            else
            {
                ...
                // Send an event to google analytics for the form submission
                _gaq.push(['_trackEvent', 'events', 'createRideError']);
            }
    ...
    MaybeSubmitForm : function() {
        ...
        if (event.canSubmitForm() && event._.formBlock.isFormSubmitPending) {
            $(event._.el.eventRideContainer).block();
            $(event._.el.rideForm).ajaxSubmit(event.formAjaxOptions);
        }
    }
    
});

Class.addSingleton(Rootless.Map.Event);
